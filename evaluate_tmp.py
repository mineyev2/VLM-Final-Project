...
(Updated with GLOBAL/WORLD Coordinate System)
...
def visualize_trajectories(image_pil, gt_waypoints_2d, pred_waypoints_2d, cam_to_ego, ego_to_world, idx, output_dir):
    ...
    Note: gt_waypoints_2d and pred_waypoints_2d are NOW in GLOBAL/WORLD frame coordinates.
    No transformation needed - they're already in world coordinates.
    """
    ...
    try:
        # Project world coordinates to image (no transformation needed, already in world frame)
...
class EvalNuScenes:
    ...
    def get_item(self, idx):
        ...
        # 4. Keep in Global/World Coordinates (NO transformation to local)
        # Instead of transforming to ego-centric, we keep world coordinates
        history_global = []
        for p in history_points:
            # Just take XY from world coordinates (already in meters)
...
def evaluate(args):
    ...
    fieldnames = ['idx', 'num_valid_waypoints', 'format_compliant', 'ade', 'fde',
                 'failure_rate', 'error_at_1s', 'error_at_2s', 'error_at_3s', 'processing_time_sec'] + \
    ...
    for batch_idx in tqdm(range(num_batches), desc='Evaluating'):
        ...
        for i, idx in enumerate(batch_indices):
            ...
            if idx in vis_indices or idx < 5:
                ...
                print(colored("Input History (Last 3 Positions in GLOBAL/WORLD frame):", "cyan"))
                for h_idx, pos in enumerate(history):
                    print(f"  t-{3-h_idx}: [{pos[0]:.2f}, {pos[1]:.2f}] (world coordinates)")

                print(colored(f"Errors:", "cyan"))
                print(f"  ADE: {ade:.4f} meters")
                print(f"  FDE: {fde:.4f} meters")

                print(colored("Future Predictions (Filtered, in GLOBAL/WORLD frame):", "cyan"))
            ...
            # Progressive failure thresholds
            error_at_1s = l2_per_waypoint[1] if len(l2_per_waypoint) > 1 else np.nan  # wp1 = 1.0s
            error_at_2s = l2_per_waypoint[3] if len(l2_per_waypoint) > 3 else np.nan  # wp3 = 2.0s
            error_at_3s = l2_per_waypoint[5] if len(l2_per_waypoint) > 5 else np.nan  # wp5 = 3.0s
            
            # Mark as failure if ANY threshold is exceeded
            failure_rate = False
            if np.isnan(error_at_1s) or error_at_1s > 10.0:
                failure_rate = True
            elif np.isnan(error_at_2s) or error_at_2s > 20.0:
                failure_rate = True
            elif np.isnan(error_at_3s) or error_at_3s > 30.0:
                failure_rate = True
            ...
            result = {
                ...
                'failure_rate': failure_rate, # True if error at 1s>10m OR 2s>20m OR 3s>30m OR any is NaN
                'error_at_1s': error_at_1s,
                'error_at_2s': error_at_2s,  # Added for tracking
                'error_at_3s': error_at_3s,  # Added for tracking
    ...
    # 
    # Failure criteria (progressive thresholds):
    #   - error_at_1s (wp1) > 10m  OR
    #   - error_at_2s (wp3) > 20m  OR
    #   - error_at_3s (wp5) > 30m  OR
    #   - any of these is NaN
    #
    # Statistics are computed ONLY on successful samples (non-failed, non-NaN)
    ...
    # Error at specific time horizons (from successful samples, excluding NaN)
    ...
    print(colored("\nFailure Criteria (any of):", 'yellow'))
    print("  • Error @ 1s > 10m")
    print("  • Error @ 2s > 20m")
    print("  • Error @ 3s > 30m")
    print(f"\nFormat Compliance: {summary['format_compliance_rate']:.2%}")
    print(colored("\nTrajectory Metrics (successful samples only):", 'cyan'))
...
def parse_args():
    ...
    parser.add_argument('--lidar_encoder_path', type=str, default='Lidar-CLIP/vit_l_14.ckpt')
